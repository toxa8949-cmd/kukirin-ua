...
export default async function BlogArticlePage({
  ...
  return (
    <article className="mx-auto max-w-3xl px-4 py-10 sm:py-14">
      <Link
        href="/blog"
        className="mb-6 inline-flex items-center gap-1 text-xs text-[#4A4A48] dark:text-white/55 hover:text-[#1a1a1a] dark:hover:text-[#1a1a1a] dark:text-white"
      >
            ...
            {related.map((r) => (
              <li key={r.id}>
                ...
                >
                  ...
                  <div className="p-3">
                    <div className="line-clamp-2 text-sm font-medium leading-snug group-hover:text-[#993C1D] dark:text-[#FF8A33]">
                      {r.title}
                    </div>
                  </div>
                </Link>
