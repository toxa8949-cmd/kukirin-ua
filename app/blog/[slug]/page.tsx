...
export default async function BlogArticlePage({
  ...
  return (
    <article className="mx-auto max-w-3xl px-4 py-10 sm:py-14">
      <Link
        href="/blog"
        className="mb-6 inline-flex items-center gap-1 text-xs text-white/55 hover:text-white"
      >
      ...
      {article.excerpt && (
        <p className="mt-4 text-lg text-white/65">{article.excerpt}</p>
      )}
      ...
      {html ? (
        ...
        <p className="mt-10 text-sm italic text-white/40">У статті поки немає тексту.</p>
      )}

      {related.length > 0 && (
        <section className="mt-16 border-t border-white/10 pt-10">
          <div className="mb-4 text-[10px] tracking-[0.2em] text-[#FF8A33]">// ЩЕ ПОЧИТАТИ</div>
          <ul className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            {related.map((r) => (
              <li key={r.id}>
                <Link
                  href={`/blog/${r.slug}`}
                  className="group block overflow-hidden rounded-sm border border-white/10 bg-[#0F0F0F] transition hover:border-[#FF6B00]/40"
                >
                  ...
                  <div className="p-3">
                    <div className="line-clamp-2 text-sm font-medium leading-snug group-hover:text-[#FF8A33]">
                      {r.title}
                    </div>
                  </div>
                </Link>
