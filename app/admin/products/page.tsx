    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <div className="mt-2 mb-1 text-[10px] tracking-[0.2em] text-[#FF8A33]">// PRODUCTS</div>
          <h1 className="text-3xl font-medium tracking-tight">Товари</h1>
          <p className="mt-2 text-sm text-white/55">{list.length} товарів</p>
        </div>
        <Link
              ...
              return (
                <li key={p.id} className="flex items-center gap-4 p-3">
                  ...
                  <div className="flex flex-shrink-0 items-center gap-1">
                    {p.is_active === false && (
                      <span className="rounded-sm border border-white/10 bg-white/5 px-1.5 py-0.5 text-[9px] uppercase tracking-wider text-white/50">
                        off
                      </span>
                    )}
                    {p.featured && (
                      <span className="rounded-sm border border-[#FF6B00]/30 bg-[#FF6B00]/10 px-1.5 py-0.5 text-[9px] uppercase tracking-wider text-[#FF8A33]">
                        feat
                      </span>
                    )}
                  </div>
