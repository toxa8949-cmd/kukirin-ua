    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <div className="mt-2 mb-1 text-[10px] tracking-[0.2em] text-[#993C1D] dark:text-[#FF8A33]">// CATEGORIES</div>
          <h1 className="text-3xl font-medium tracking-tight">Категорії</h1>
          <p className="mt-2 text-sm text-[#4A4A48] dark:text-white/55">{list.length} категорій</p>
        </div>
        <Link
      ...
      <div className="rounded-sm border border-[#E8E6DE] dark:border-white/10 bg-white dark:bg-[#0F0F0F]">
        {list.length === 0 ? (
          <div className="p-8 text-center text-sm text-[#4A4A48] dark:text-white/55">Категорій ще немає.</div>
        ) : (
              ...
              return (
                <li key={c.id} className="flex items-center gap-4 p-3">
                  <div className="min-w-0 flex-1">
                    ...
                    {c.description && (
                      <div className="mt-1 line-clamp-1 text-xs text-[#6C6A65] dark:text-white/50">{c.description}</div>
                    )}
                  </div>
                  <Link
