      ...
      <header className="sticky top-0 z-50 border-b border-white/10 bg-[#0A0A0A]/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-10">
          <div className="flex items-center gap-4 md:gap-8">
            ...
            <ul className="hidden gap-5 text-sm text-white/70 md:flex">
              <li><Link href="/catalog" className="hover:text-white">Самокати</Link></li>
              <li><Link href="/accessories" className="hover:text-white">Аксесуари</Link></li>
              <li><Link href="/service" className="hover:text-white">Сервіс</Link></li>
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <details className="relative md:hidden">
              ...
              <div className="absolute right-0 mt-3 w-48 rounded-sm border border-white/10 bg-[#111] p-3 text-sm shadow-xl">
                <Link href="/catalog" className="block py-2 text-white/80 hover:text-white">Самокати</Link>
                <Link href="/accessories" className="block py-2 text-white/80 hover:text-white">Аксесуари</Link>
                <Link href="/service" className="block py-2 text-white/80 hover:text-white">Сервіс</Link>
                <Link href="/blog" className="block py-2 text-white/80 hover:text-white">Блог</Link>
                <Link href="/contacts" className="block py-2 text-white/80 hover:text-white">Контакти</Link>
              </div>
            </details>
          </div>
        </div>
      ...
      {(title || breadcrumb) && (
        <section className="border-b border-white/10 bg-[#0A0A0A]">
          <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14 lg:px-10">
            {breadcrumb && (
              <div className="mb-3 text-[10px] tracking-[0.2em] text-[#FF8A33]">// {breadcrumb}</div>
            )}
            {title && (
              <h1 className="text-3xl font-medium leading-[1.05] tracking-[-0.03em] sm:text-4xl md:text-5xl">{title}</h1>
            )}
            {subtitle && (
              <p className="mt-3 max-w-2xl text-sm text-white/55 sm:text-base">{subtitle}</p>
            )}
          </div>
        </section>
      )}
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
          <div className="grid grid-cols-2 gap-8 border-b border-white/10 pb-10 md:grid-cols-4">
            <div className="col-span-2 md:col-span-1">
              ...
              <p className="mb-4 max-w-xs text-xs leading-relaxed text-white/45">Офіційний дистрибʼютор електросамокатів KUKIRIN в Україні. Гарантія, сервіс, доставка.</p>
              <div className="flex gap-2">
                <a href="#" aria-label="Instagram" className="flex h-8 w-8 items-center justify-center rounded-sm border border-white/10 text-white/60 hover:border-[#FF6B00] hover:text-[#FF6B00]"><Instagram size={14} /></a>
                <a href="#" aria-label="Facebook" className="flex h-8 w-8 items-center justify-center rounded-sm border border-white/10 text-white/60 hover:border-[#FF6B00] hover:text-[#FF6B00]"><Facebook size={14} /></a>
                <a href="#" aria-label="YouTube" className="flex h-8 w-8 items-center justify-center rounded-sm border border-white/10 text-white/60 hover:border-[#FF6B00] hover:text-[#FF6B00]"><Youtube size={14} /></a>
                <a href="#" aria-label="Telegram" className="flex h-8 w-8 items-center justify-center rounded-sm border border-white/10 text-white/60 hover:border-[#FF6B00] hover:text-[#FF6B00]"><Send size={14} /></a>
              </div>
            </div>
            <div>
          ...
          <div className="flex flex-col items-start justify-between gap-2 pt-6 text-[11px] text-white/40 md:flex-row md:items-center">
            ...
            <div className="flex gap-4">
              ...
              <Link href="/terms" className="hover:text-white">Угода</Link>
            </div>
          </div>
        </div>
      </footer>
