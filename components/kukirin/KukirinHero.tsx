    ...
    <section className="relative overflow-hidden bg-[#0A0A0A] text-white">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 lg:px-10">
        ...
        <nav className="flex items-center justify-between border-b border-white/10 py-4">
          <div className="flex items-center gap-8">
            ...
            <ul className="hidden gap-5 text-sm text-white/70 md:flex">
              <li><a href="#models" className="hover:text-white">Самокати</a></li>
              <li><a href="/accessories" className="hover:text-white">Аксесуари</a></li>
              <li><a href="/service" className="hover:text-white">Сервіс</a></li>
              <li><a href="/blog" className="hover:text-white">Блог</a></li>
            </ul>
          </div>
          <div className="flex items-center gap-4">
            <button aria-label="Пошук" className="text-white/80 hover:text-white">
              <Search size={18} />
            </button>
            <a href="/account" aria-label="Кабінет" className="text-white/80 hover:text-white">
              <User size={18} />
            </a>
            <a href="/cart" aria-label="Кошик" className="text-white/80 hover:text-white">
        ...
        <div className="relative py-16 lg:py-20">
          {/* Індикатор наявності */}
          <div className="kukirin-fade-1 absolute right-0 top-12 flex items-center gap-2 text-[11px] tracking-[0.15em] text-white/60">
            <span className="kukirin-pulse-dot" />
            В НАЯВНОСТІ · 12 МОДЕЛЕЙ
          </div>

          {/* Системний тег */}
          <div className="kukirin-fade-1 mb-5 text-[11px] tracking-[0.3em] text-[#FF8A33]">
          ...
          <h1 className="kukirin-fade-3 mb-5 text-5xl font-medium leading-[0.9] tracking-[-0.04em] text-[#FF6B00] md:text-7xl lg:text-8xl">
            <span className="kukirin-glitch relative inline-block" data-text="RUSH.">
              RUSH<span className="text-white">.</span>
            </span>
            <span className="kukirin-cursor text-[#FF6B00]">_</span>
          </h1>

          {/* Опис */}
          <p className="kukirin-fade-3 mb-6 max-w-md text-sm leading-relaxed text-white/55">
          ...
          <div className="kukirin-fade-4 mb-9 flex flex-wrap gap-2">
            <a
              href="/product/g2-pro"
              className="inline-flex items-center gap-2 rounded-sm bg-[#FF6B00] px-6 py-3 text-xs font-medium tracking-[0.1em] text-black transition hover:bg-[#FF8A33]"
            >
              ЗАМОВИТИ G2 PRO
              <ArrowRight size={14} />
            </a>
            <a
              href="/test-drive"
              className="rounded-sm border border-white/25 px-6 py-3 text-xs font-medium tracking-wide text-white transition hover:border-white/50"
            >
              Тест-драйв
            </a>
          </div>
          ...
          {/* Статистика */}
          <div className="kukirin-fade-4 grid grid-cols-2 gap-6 border-t border-white/10 pt-6 md:grid-cols-4">
            {HERO_STATS.map((stat) => (
              <div key={stat.label}>
