...
export default function CheckoutForm() {
  ...
  if (!mounted) {
    return (
      <div className="rounded-sm border border-white/10 bg-[#0F0F0F] p-8 text-sm text-white/55">
        Завантаження…
      </div>
    );
  }
  ...
  if (count === 0) {
    return (
      <div className="mx-auto max-w-lg rounded-sm border border-white/10 bg-[#0F0F0F] p-8 text-center sm:p-12">
        ...
        <p className="mb-5 text-sm text-white/55">Спочатку додайте товар у кошик.</p>
        <Link
          href="/catalog"
          className="inline-flex items-center gap-2 rounded-sm bg-[#FF6B00] px-6 py-3 text-xs font-medium tracking-[0.1em] text-black hover:bg-[#FF8A33]"
        >
  ...
  const inputCls =
    'w-full rounded-sm border border-white/10 bg-[#0F0F0F] px-3 py-2.5 text-sm outline-none transition focus:border-[#FF6B00]';

  return (
    <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_360px]">
      {/* Form */}
      <div className="space-y-6">
        <fieldset className="space-y-3 rounded-sm border border-white/10 bg-[#0F0F0F] p-5">
          <legend className="px-2 text-[10px] tracking-[0.2em] text-[#FF8A33]">// КОНТАКТНІ ДАНІ</legend>
          <label className="block">
            <span className="mb-1 block text-xs text-white/55">Імʼя та прізвище*</span>
            <input
              type="text"
              autoComplete="name"
          ...
          <label className="block">
            <span className="mb-1 block text-xs text-white/55">Телефон*</span>
            <input
              type="tel"
              autoComplete="tel"
          ...
          <label className="block">
            <span className="mb-1 block text-xs text-white/55">Email (за бажанням)</span>
            <input
              type="email"
              autoComplete="email"
        ...
        <fieldset className="space-y-3 rounded-sm border border-white/10 bg-[#0F0F0F] p-5">
          <legend className="px-2 text-[10px] tracking-[0.2em] text-[#FF8A33]">// ДОСТАВКА</legend>
          <label className="block">
            <span className="mb-1 block text-xs text-white/55">Спосіб доставки*</span>
            <select
          ...
          <label className="block">
            <span className="mb-1 block text-xs text-white/55">Місто та адреса/відділення*</span>
            <input
              type="text"
        ...
        <fieldset className="space-y-3 rounded-sm border border-white/10 bg-[#0F0F0F] p-5">
          <legend className="px-2 text-[10px] tracking-[0.2em] text-[#FF8A33]">// ОПЛАТА</legend>
          <label className="block">
            <span className="mb-1 block text-xs text-white/55">Спосіб оплати*</span>
            <select
          ...
          <label className="block">
            <span className="mb-1 block text-xs text-white/55">Коментар до замовлення</span>
            <textarea
              rows={3}
              value={form.notes}
      ...
      {/* Summary */}
      <aside className="h-fit space-y-4 rounded-sm border border-white/10 bg-[#0F0F0F] p-5">
        <div className="text-[10px] tracking-[0.2em] text-[#FF8A33]">// ВАШЕ ЗАМОВЛЕННЯ</div>
        <ul className="space-y-2 text-sm">
          {items.map((it) => (
            <li key={it.slug} className="flex items-start justify-between gap-3 border-b border-white/5 pb-2">
              <div>
                <div className="text-sm">{it.name}</div>
                <div className="text-xs text-white/45">{it.quantity} × {it.price.toLocaleString('uk-UA')} ₴</div>
              </div>
        ...
        <dl className="space-y-1 text-sm">
          ...
          <div className="mt-2 flex justify-between border-t border-white/10 pt-2 text-base font-medium">
            <dt>До сплати</dt>
